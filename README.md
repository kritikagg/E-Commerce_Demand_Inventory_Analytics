...
## 6. Methodology
1. Generate synthetic transactional data with realistic business patterns
2. Clean data (deduplication, missing values, invalid-value checks, type conversion)
3. Engineer derived fields (Revenue, NetRevenue, Profit, date parts)
4. Load into SQL Server star schema; validate with business queries
5. Compute per-product demand statistics and classify demand behavior
6. Forecast 30-day demand for the top 10 revenue products using exponential smoothing (selected via backtest against naive and moving-average baselines)
7. Score inventory risk using safety stock / reorder point formulas
8. Build a 3-page Power BI dashboard
9. Derive business insights and recommendations from the actual results
...
- 11 of the top 20 revenue products are currently below their computed reorder point
- Festive-season (Oct–Dec) monthly demand runs ~40% above other months
- 3 of 17 slow-moving products also carry excess inventory (>60 days coverage) — a real but smaller pattern than initially expected
...
- Category-level YoY growth (matched calendar months) shows real divergence: Sports grew ~7.8% while Electronics declined ~5.9%, with the remaining categories roughly flat. Comparing matching calendar months (rather than a naive first-half-vs-second-half split) was necessary to avoid a seasonal confound, since the first half of the date range included the Oct–Dec festive spike and the second half didn't

## 13. Business Recommendations
1. Prioritize supply reliability and inventory buffers for the top-10 revenue products
2. Trigger immediate replenishment review for the 11 at-risk top-20 products
3. Build seasonal (Q4) safety-stock buffers rather than a flat year-round policy
4. Run clearance/markdown campaigns on the smaller set of slow-moving, excess-inventory SKUs
5. Validate discount ROI against margin impact before scaling promotions
6. Investigate the Central region's lower revenue share (demand vs. fulfillment gap)
7. Look into Electronics' YoY decline and Sports' growth to understand what's driving the divergence (pricing, competition, assortment) before setting next year's category targets
